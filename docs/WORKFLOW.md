...
代码写完了吗?                           否 → 让 AI 按任务清单实现
    ↓ 是
有设计截图吗?                           是 → /visual-qa (自动) 或 /code 末尾已自动跑
    ↓ 无或已通过
测试生成过了吗?                         否 → /test <目录>
    ↓ 是
审查过了吗?                             否 → /review <目录>
...
1. 检测 `Spider` app 是否在运行
   - **运行中** → 用 AppleScript 激活窗口，截取真实 Tauri 窗口截图
   - **未运行** → 自动起 web-only Vite 服务，用 Playwright + mock 数据截图
...
4. 产出文件到 `docs/designs/screenshots/actual/`：
   - `actual-tauri.png` / `actual-spider.png` — 本次截图
   - `diff.png` — 红色高亮差异区域
...
- P0/P1 有差异时 AI 会自动定位到 `.module.css` / `.tsx` 修复，最多 3 轮
- P2 差异记录在 JSON 里，不影响进度

**设计截图放哪**：把设计稿导出的截图放到 `docs/designs/screenshots/reference/` 即可自动识别。

**常见卡点**：
- ❓ 截 Tauri 窗口失败 → 系统偏好 → 隐私与安全 → 屏幕录制 → 允许 Terminal
- ❓ RMSE 偏高但视觉上差不多 → 正常，内容差异（空泳道 vs 有卡片）会拉高 RMSE，看 side-by-side 做最终判断
- ❓ 想只跑截图不对比 → `bash docs/designs/screenshots/capture-window.sh`
...
| 编码完想看和设计稿差多少 | `/visual-qa` — 自动截图+对比，输出 P0/P1/P2 报告 |
| 只想截一张真实窗口图 | `bash docs/designs/screenshots/capture-window.sh` |
...
| 全部命令 | 主流程 [`../.claude/commands/`](../.claude/commands/): `/prd` `/prd-check` `/plan` `/plan-check` `/code` `/test` `/review` `/bug-check` `/fix` `/release` `/build` `/deploy` `/start` `/meta-audit` `/visual-qa` |
