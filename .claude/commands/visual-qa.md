...
最小改动:
  ...
  4. /code @docs/tasks/tasks-xxx.json --only <受影响 taskId>（重新实现该任务文件）
  5. /visual-qa 验证
```
...
最小改动:
  ...
  2. /code @docs/tasks/tasks-xxx.json --only <taskId>
     （仅重跑该任务，让 @design 写入源文件 JSDoc 并按设计图重新实现）
  3. /visual-qa 验证
```
...
最小改动:
  ...
  2. 读 side-by-side.png 肉眼对比，提取期望值（高度/颜色/间距）
  3. Grep 找到对应 .module.css:
       grep -r "区域名" workspace/src --include="*.module.css" -l
...
| 差异类型 | 原因 | 处置 |
|---------|------|------|
| 空泳道 vs 设计稿有卡片 | 数据内容差异，预期行为 | 报告注明，不修复 |
| 字体渲染微差（P2） | macOS 子像素抗锯齿 vs 设计工具 | P2 接受 |
