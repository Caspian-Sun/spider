...
1. **启动前端 dev 服务**:
   ...
   pnpm dev:web-only &
   # 等待端口就绪 (最多 15s)
   until curl -s http://localhost:1420 > /dev/null; do sleep 1; done
   ```
...
3. **用 Playwright 截图实际效果**:
   ...
   npx playwright screenshot --browser chromium \
     --full-page http://localhost:1420 \
     /tmp/actual-screenshot.png
   ```

3. **读取两张图像对比** — 调用 Read 工具分别读取设计截图 (`docs/designs/screenshots/*.png`) 和实际截图 (`/tmp/actual-screenshot.png`), 用视觉判断做三层检查:

   | 层级 | 检查项 | 合格标准 | 不合格时 |
   |------|--------|---------|---------|
   | **P0 — 结构** | 主要区域存在 (ActivityBar / TopBar / Board 泳道 / RetroTimeline) | 100% 存在 | **必须修复再继续** |
...
6. **追溯根因到命令文件** — 如果差异是因为 `/plan` 生成的 tasks.json `businessRules` 描述不精确, 或 `/prd` 的设计规则有误导, 同步修复对应的 `.claude/commands/` 文件里相关描述, 避免下次 `/code` 重蹈覆辙。具体判断:
